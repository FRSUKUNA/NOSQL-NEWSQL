...
def main():
    ...
    versions.sort(key=lambda x: [int(n) for n in x['full_version'].split('.')], reverse=True)
    output_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', '..', 'API', 'sources'))
    # Sauvegarder dans un fichier JSON
    output_file = os.path.join(output_dir, "cassandra-versions.json")
    print("Chemin absolu:", os.path.abspath(output_file))
